...
int main( int argc, char *argv[]){
    ...
    CStrExplode client_listent_ip_list(client_listen_ip, ';');
    for( uint32_t i = 0; i < client_listent_ip_list.GetItemCnt(); ++i)
    {
        ret = netlib_listen(client_listent_ip_list.GetItem(i), client_port, client_callback, NULL);
        if( NETLIB_ERROR == ret )
        {
            return ret;
        }

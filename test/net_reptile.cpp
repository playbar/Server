                     ...
                     char *&response,int &bytesRead)
{
    ...
    uint32_t sock = socket( AF_INET, SOCK_STREAM,
                         IPPROTO_TCP);
    if( sock == -1 || sock == -2 )
    {
...
//    if(!CreateDirectory( foldname.c_str(),NULL ))
    if(mkdir(foldname.c_str(), S_IRWXU|S_IRWXG|S_IRWXO) != 0)
       std::cout << "Can not create directory:" << foldname.c_str()<<endl;
    char *image;
    int byteRead;
...
int main()
{
    ...
    // 遍历的起始地址
    string urlStart = "http://www.csdn.net/";
    // 使用广度遍历
    // 提取网页中的超链接放入hrefUrl中，
    //提取图片链接，下载图片。
    BFS( urlStart );

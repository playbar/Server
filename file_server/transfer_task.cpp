...
static FILE* OpenByRead(const std::string& task_id, uint32_t user_id)
{
    ...
    if (task_id.length()>=2)
    {
        ...
        snprintf(save_path, BUFSIZ, "%s/%s/%s", GetCurrentOfflinePath(),
                 task_id.substr(0, 2).c_str() , task_id.c_str());
...
//----------------------------------------------------------------------------
BaseTransferTask::BaseTransferTask(const std::string& task_id, uint32_t from_user_id,
...
int OnlineTransferTask::DoRecvData(uint32_t user_id, uint32_t offset, const char* data,
                                   uint32_t data_size)
{
    int rv = -1;
...
int OnlineTransferTask::DoPullFileRequest(uint32_t user_id, uint32_t offset,
...
int OfflineTransferTask::DoRecvData(uint32_t user_id, uint32_t offset, const char* data,
                                    uint32_t data_size)
{
    // 离线文件上传
    int rv = -1;
...
int OfflineTransferTask::DoPullFileRequest(uint32_t user_id, uint32_t offset,
                                           uint32_t data_size, std::string* data)
{
    int rv = -1;

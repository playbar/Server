...
void FileMsgServerConnTimerCallback(void *callback_data, uint8_t msg, uint32_t handle, void *param)
{
...
FileMsgServerConn::FileMsgServerConn() : connected_(false)
{
...
void FileMsgServerConn::HandlePdu(CImPdu *pdu)
{
    switch (pdu->GetCommandId())
    {
        ...
        case CID_OTHER_FILE_SERVER_IP_RSP:
            ...
            break;
        default:
            log("No such cmd id = %d", pdu->GetCommandId());
            break;
    }
    return;
}
...
void FileMsgServerConn::_HandleMsgFileTransferReq(CImPdu *pdu)
{
    ...
    do
    {
        ...
        BaseTransferTask* transfer_task = TransferTaskManager::GetInstance()->NewTransferTask(
            transfer_req.trans_mode(), task_id, from_id,  to_id, transfer_req.file_name(),

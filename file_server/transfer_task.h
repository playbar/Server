...
{
public:
    BaseTransferTask(const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id,
                     const std::string& file_name, uint32_t file_size);
    ...
    bool IsWaitTranfering() const
    {
        ...
        if (state_ == kTransferTaskStateWaitingTransfer ||
            state_ == kTransferTaskStateWaitingUpload || kTransferTaskStateWaitingDownload)
        {
            rv = true;
        }
        return rv;
...
{
public:
    OnlineTransferTask(const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id,
                       const std::string& file_name, uint32_t file_size)
    ...
    virtual int DoRecvData(uint32_t user_id, uint32_t offset, const char* data, uint32_t data_size);
    virtual int DoPullFileRequest(uint32_t user_id, uint32_t offset, uint32_t data_size,
                                  std::string* data);
...
{
public:
    OfflineTransferTask(const std::string& task_id, uint32_t from_user_id, uint32_t to_user_id,
                        const std::string& file_name, uint32_t file_size)
    : BaseTransferTask(task_id, from_user_id, to_user_id, file_name, file_size)
    {
        //        file_header_ = NULL;

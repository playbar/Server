...
{
    PUSH_SERVER_WARN("apns gateway client has exception, sockid: %u, error code: %d.",
                     nsockid, nErrorCode);
    apns_client_ptr pClient = CSessionManager::GetInstance()->GetAPNSClient();
    if (pClient)
    {
...
void CAPNSClientHandler::OnRecvData(const char* szBuf, int32_t nBufSize)
{
    ...
    while (m_Msg.CheckMsgAvailable())
    {
        ...
        {
            PUSH_SERVER_INFO("apns gateway client recv resp, cmd id: %u, status: %u, notification id: %u",
                    (uint32_t)msg.GetCommandID(), (uint32_t)msg.GetStatus(), msg.GetNotificationID());
            //apns_client_ptr pClient = CSessionManager::GetInstance()->GetAPNSClient();
            //if (pClient)
            //{
            //    pClient->StopGateWayClient();
